...
from MYNewAiml import DefaultSubs
from MYNewAiml import Utils, DefaultSubs
...
class Kernel:
        ...
        try:
            ...
            for s in sentences:
                ...
                if len(outputHistory)>1:
                    if ("$$rresume$$" in outputHistory[-1] and ("$$valid$$") not in outputHistory[-2] and (("$$invalid$$") not in outputHistory[-2])) :
                        print("GOT A QUESTION REMEMBRANCE ONE FROM AIML KERNEL")
                        for i in range(0, 3):
                            del outputHistory[-1]

                    while ("$$rresumestop$$" in outputHistory[-1]):
                        del outputHistory[-1]
                    while ("$$rresume$$" in outputHistory[-1]):
                        del outputHistory[-1]
    ...
    def _respond(self, input_, sessionID):
        ...
        print ("MY SUBBED INPUT IS: ",subbedInput)
