...
def decode_entities(s):
    ...
    if isinstance(s, str):
        return RE_UNICODE.subn(replace_entity, s)[0]
    return s
...
def find(match=lambda item: False, item_list=None):
...
def order(value_list, cmp=None, key=None, reverse=False):
    ...
    if cmp and key:
        def f(i, j):
            return cmp(key(value_list[i]), key(value_list[j]))
    elif cmp:
        def f(i, j):
            return cmp(value_list[i], value_list[j])
    elif key:
        def f(i, j):
            return int(key(value_list[i]) >= key(value_list[j])) * 2 - 1
    else:
        def f(i, j):
            return int(value_list[i] >= value_list[j]) * 2 - 1
    return sorted(value_list(range(len(value_list))), cmp=f, reverse=reverse)
...
class Database(object):
    ...
    def __init__(self, name, host="localhost", port=3306, username="root", password="", type=SQLITE, str=True,
                 **kwargs):
        ...
        if self.type == MYSQL:
            try:
                self._connection = MySQLdb.connect(
                    self.host, self.username, self.password, self.name, port=self.port, use_unicode=str)
...
class Table(object):
    ...
    def insert(self, *args, **kwargs):
        ...
        if len(args) == 0 and len(kwargs) == 1 and isinstance(kwargs.get("values"), dict):
            ...
            kwargs = dict(
                list(zip((f for f in self.fields if f != self.pk), args[0])))
...
class FilterChain(list):
    def __init__(self, *args, **kwargs):
        ...
        args.extend(list(filter(k, v, "=")) for k, v in list(kwargs.items()))
...
class Query(object):
    ...
    def __init__(self, table, fields=ALL, filters=[], relations=[], sort=None, order=ASCENDING, group=None,
                 function=FIRST, range=None):
    ...
    def group(self, j, function=FIRST, key=lambda v: v):
        ...
        for v in g:
            ...
            u[o[v]] = [[list.__getitem__(self, i)[j]
                        for i in g[v]] for j in range(self._m)]
