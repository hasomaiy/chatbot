...
from mypattern.pattern.text.tree import Tree, Text, Sentence, Slice, Chunk, PNPChunk, Chink, Word, table
from mypattern.pattern.text.tree import SLASH, WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA, AND, OR
...
RE_EMOTICONS = [r" ?".join(map(re.escape, e))
                for v in list(EMOTICONS.values()) for e in v]
...
class Sentiment(lazydict):
    ...
    def load(self, path=None):
        ...
        for w in words:
            words[w] = dict((pos, [avg(x) for x in zip(*psi)])
                            for pos, psi in list(words[w].items()))
        # Average scores of all part-of-speech tags.
        for w, pos in list(words.items()):
            words[w][None] = [avg(x) for x in zip(*list(pos.values()))]
    ...
    def __call__(self, s, negation=True, ngrams=DEFAULT, **kwargs):
        ...
        return Score(polarity=avg([(w[0], w[1]) for w in a], weight),
                     subjectivity=avg([(w[0], w[2]) for w in a], weight),
                     assessments=a)
    ...
    def assessments(self, words=[], negation=True, ngrams=DEFAULT):
        ...
        while index < len(words):
            ...
            if w is None:
                index += 1
                continue
            for i in reversed(list(range(1, max(1, ngrams)))):
            ...
            if w in self and pos in self[w]:
                ...
                if w.isalpha() is False and len(w) <= 5 and w not in PUNCTUATION:
                    for E in (EMOTICONS, EMOJI):
                        for (type, p), e in list(E.items()):
                            if w in [e.lower() for e in e]:
                                a.append(
    ...
    def save(self, path):
        ...
        for w in sorted(self):
            for pos, (p, s, i) in list(self[w].items()):
                pos = pos or ""
