...
class TestUnicode(unittest.TestCase):
    ...
    def test_decode_utf8(self):
        ...
        for s in self.strings:
            self.assertTrue(isinstance(web.decode_string(s), str))
        print("pattern.web.decode_utf8()")

    def test_encode_utf8(self):
        # Assert Python bytestring.
        for s in self.strings:
            self.assertTrue(isinstance(web.encode_string(s), str))
...
class TestURL(unittest.TestCase):
    ...
    def test_url_open(self):
        ...
        v = web.URL(self.live)
        self.assertIsNotNone(v.open())
        self.assertEqual(v.exists, True)
        # Assert user-agent and referer.
        self.assertIsNotNone(v.open(user_agent=web.MOZILLA, referrer=web.REFERRER))
    ...
    def test_oauth(self):
        ...
        v = web.oauth.sign(
            "http://yboss.yahooapis.com/ysearch/web", data, secret="secret")
...
class TestPlaintext(unittest.TestCase):
    ...
    def test_find_urls(self):
        ...
        self.assertEqual(
            web.find_urls("HTTP://domain.net")[0], "HTTP://domain.net")
        self.assertEqual(web.find_urls("http://domain.net),};")
                         [0], "http://domain.net")
        self.assertEqual(
            web.find_urls("http://domain.net\">domain")[0], "http://domain.net")
        self.assertEqual(
            web.find_urls("domain.com, domain.net"), ["domain.com", "domain.net"])
    ...
    def test_strip_tags(self):
        ...
        v = web.strip_tags(
            "<a href=\"\" onclick=\"\">text</a>", exclude={"a": ["href"]})
...
class TestSearchEngine(unittest.TestCase):
    ...
    def _test_search_engine(self, api, source, license, Engine, query="today", type=web.SEARCH):
        ...
        # except Wikipedia which returns a WikipediaArticle (MediaWikiArticle
        # subclass).
        ...
        self.assertRaises(
            web.SearchEngineTypeError, e.search, query, type="crystall-ball")
    ...
    def test_search_wikia(self):
        self._test_search_engine(
            "Wikia", *self.api["Wikia"], **{"query": "games"})

    def test_search_flickr(self):
        self._test_search_engine(
            "Flickr", *self.api["Flickr"], **{"type": web.IMAGE})
    ...
    def _test_results(self, api, source, license, Engine, type=web.SEARCH, query="today", baseline=[6, 3, 6, 0]):
        ...
        # query in url + title + description
        self.assertTrue(i2 >= baseline[1])
        self.assertTrue(i3 >= baseline[2])  # language "en"
        # url's ending with "jpg", "png" or "gif"
        self.assertTrue(i4 >= baseline[3])
    ...
    def test_results_yahoo_images(self):
        self._test_results(
            "Yahoo", *self.api["Yahoo"], **{"type": web.IMAGE, "baseline": [6, 6, 0, 6]})

    def test_results_yahoo_news(self):
        self._test_results(
            "Yahoo", *self.api["Yahoo"], **{"type": web.NEWS})
    ...
    def test_results_bing_images(self):
        self._test_results(
            "Bing", *self.api["Bing"], **{"type": web.IMAGE, "baseline": [6, 6, 0, 6]})
    ...
    def test_results_flickr(self):
        self._test_results(
            "Flickr", *self.api["Flickr"], **{"baseline": [6, 6, 0, 6]})

    def test_results_facebook(self):
        self._test_results(
            "Facebook", *self.api["Facebook"], **{"baseline": [0, 1, 0, 0]})
    ...
    def _test_search_image_size(self, api, source, license, Engine):
        ...
        for size in (web.TINY, web.SMALL, web.MEDIUM, web.LARGE):
            try:
                v = e.search(
                    "cats", type=web.IMAGE, count=1, size=size, cached=False)
            except web.HTTP403Forbidden:
                raise unittest.SkipTest("FIXME")
            self.assertEqual(web.URL(v[0].url).exists, True)
            print("pattern.web.%s.search(type=IMAGE, size=%s)" %
                  (api, size.upper()))
    ...
    def test_wikipedia_all(self):
        # Assert WikipediaArticle.all(), an iterator over WikipediaArticle
        # objects.
        source, license, Engine = self.api["Wikipedia"]
    ...
    def test_wikipedia_article_sections(self):
        ...
        # The test may need to be modified if the Wikipedia "Cat" article
        # changes.
        source, license, Engine = self.api["Wikipedia"]
        v = Engine(license).search("cat", cached=False)
        s1 = s2 = s3 = None
        for section in v.sections:
            ...
            self.assertTrue(
                section.level == 0 or section.string.startswith(section.title))
        # Test section depth.
...
class TestDOM(unittest.TestCase):
    ...
    def test_node_document(self):
        ...
        # Note: BeautifulSoup strips whitespace.
    ...
    def test_element(self):
        ...
        self.assertEqual(
            a[0].by_tag("span")[1].attributes["class"], ["author"])
...
class TestDocumentParser(unittest.TestCase):
    ...
    def test_pdf(self):
        ...
        s = web.parsedoc(
            os.path.join("./", "corpora", "carroll-wonderland.pdf"))
    ...
    def test_docx(self):
        # Assert PDF to string parser.
        s = web.parsedoc(
            os.path.join("./", "corpora", "carroll-lookingglass.docx"))
...
class TestMail(unittest.TestCase):
    ...
    def test_mail(self):
        ...
        m = web.Mail(self.username, self.password,
                     service=self.service, port=self.port, secure=self.SSL)
    ...
    def test_mail_message2(self):
        ...
        # Set query2 to a mail subject of a spam e-mail you know contains an
        # attachment.
        m = web.Mail(self.username, self.password,
                     service=self.service, port=self.port, secure=self.SSL)
        if "spam" in m.folders:
            for id in m.spam.search(self.query2, field=web.SUBJECT):
                ...
                if len(e.attachments) > 0:
                    ...
                    print("pattern.web.Message.attachments (MIME-type: %s)" %
                          e.attachments[0][0])
...
class TestCrawler(unittest.TestCase):
    ...
    def test_crawler_crawl(self):
        ...
        v = web.Crawler(
            links=["http://www.clips.ua.ac.be/"], domains=["clips.ua.ac.be"], delay=0.5)
    ...
    def test_crawler_delay(self):
        ...
        v = web.Crawler(
            links=["http://www.clips.ua.ac.be/"], domains=["clips.ua.ac.be"], delay=1.0)
        v.crawl()
        t = time.time()
